...
async function main() {
  const customQuery = await ethers.getContractAt("CustomQuery", "0xe3db72683FbaB6F3d685a0Ba395c18355E61ceda")
  ...
  try {
    ...
    tx = await customQuery.query(queries, { gasLimit: 1000000, value: fee.mul(120).div(100) })
    await tx.wait()
    console.log(tx)
  } catch (error) {
    console.error(error)
